...
describe("Options", () => {
  ...
  test("overriding", () => {
    ...
    expect(Options.Defaults.override({ maxAttempts: 777 }).maxAttempts).toEqual(
      777
    );
    ...
    expect(Options.Defaults.override({ log: customLog }).log).toEqual(
      customLog
    );
  });
});
...
describe("retry", () => {
  ...
  test("first invocation succeeded (generic)", async () => {
    ...
    const wrapper = retry<MockRequest, MockResponse>(
      mockSdk,
      mockSdk.invoke,
      opts
    );
